...
[![Version](https://img.shields.io/badge/version-v0.7-brightgreen.svg)](https://github.com/Luerlei/MOM-AI-KBS/releases)

基于 FastAPI + Vue3 + ChromaDB 构建，平台模型无关（统一接口抽象层），整合 **AI 知识库 + 智能问答 + 时序预测分析** 三大能力，支持 LLM 智能问答、语义/关键词搜索、Skill 路由分发、协变量增强预测、Token 统计优化等完整功能。
...
- **自实现轻量 RAG**：不依赖 LangChain/LlamaIndex，ChromaDB SDK + httpx，混合检索（向量 + BM25）+ RRF 融合
- **模型无关架构**：统一 `LLMClient` 抽象层，支持 LLM / Embedding / Forecast 三类模型
...
- **优雅降级**：无 Embedding 时 RAG 降级为直接对话；语义搜索失败自动切换关键词搜索
- **运行时热更新**：模型配置存储于数据库，Web 界面修改后立即生效
- **Token 优化**：问答缓存 + 缓存命中率统计 + Skill 路由限定检索范围
- **全量调用日志**：记录所有模型调用（chat / embedding / test / forecast），含发起人、延迟、token 消耗
...
| 文件解析 | PyPDF2 / python-docx / openpyxl | PDF / DOCX / XLSX 解析 |
...
[问答缓存] 向量相似度 > 0.90 → 命中缓存直接返回
  │
  ▼
[混合检索] 向量检索 + BM25 关键词检索 + RRF 融合（仅 published 知识）
  │
  ▼
[Prompt 组装] Skill 模板 + 检索片段 + 用户问题（Token 预算控制）
  │
  ▼
[LLM 流式生成] SSE 逐步返回答案
  │
...
| 模型配置 | /models | LLM/Embedding/Forecast 配置、连通测试、热更新 |
...
| 模型 | GET | /api/models/status | 当前激活的 LLM/Embedding/Forecast |
