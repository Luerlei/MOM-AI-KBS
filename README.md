...
[![Version](https://img.shields.io/badge/version-v0.9-brightgreen.svg)](https://github.com/Luerlei/MOM-AI-KBS/releases)

基于 FastAPI + Vue3 + ChromaDB 构建，平台模型无关（统一接口抽象层），整合 **AI 知识库 + 智能问答 + 时序预测分析** 三大能力，支持多知识库管理、会话式问答、Skill 路由分发、语义/关键词搜索、协变量增强预测、OCR/VLM 深度文档解析、Token 统计优化等完整功能。
...
- **多知识库管理**：知识库作为顶层容器，独立配置 LLM/Embedding/Rerank/OCR/VLM（空则继承全局），支持批量上传 + 解析状态机（pending/parsing/parsed/failed）★ v0.9
- **会话式问答**：ChatGPT 风格会话管理，会话关联多个知识库限定检索范围，多轮对话 + SSE 流式 + 引用溯源 ★ v0.9
- **Skill 路由引擎**：关键词/正则/语义三级匹配，使用专属 Prompt 模板和知识范围
- **自实现轻量 RAG**：不依赖 LangChain/LlamaIndex，ChromaDB SDK + httpx，混合检索（向量 + 真 BM25）+ RRF 融合，支持按知识库范围检索
...
- **全量调用日志**：记录所有模型调用（chat / embedding / test / forecast / ocr / conversation），含发起人、延迟、token 消耗
...
| **v0.9** | 2026-07-14 | **增加知识库管理（多知识库容器、独立模型配置、批量上传+解析状态机）、会话管理（ChatGPT 风格多轮对话、会话关联知识库限定检索范围、SSE 流式问答）** |
...
| 数据库 | SQLAlchemy 2.0 + SQLite | 关系型数据（19 个模型） |
...
├── backend/                          # 后端服务
│   ├── app/
│   │   ├── main.py                   # FastAPI 入口（16 个路由注册）
│   │   ├── config.py                 # 配置管理（环境变量 + 安全校验）
│   │   ├── database.py               # 数据库初始化 + 自动迁移
│   │   ├── data/                     # 静态数据
│   │   │   └── china_holidays.json   # 中国节假日数据集（2024-2026）
│   │   ├── models/                   # 19 个数据模型（含 knowledge_base / conversation ★v0.9）
│   │   ├── routers/                  # 16 个 API 路由模块（含 knowledge_base / conversation ★v0.9）
│   │   ├── schemas/                  # Pydantic 验证模型
│   │   ├── services/                 # 23 个业务服务（含 knowledge_base_service / conversation_service ★v0.9）
...
├── frontend/                         # 前端应用
│   ├── src/
│   │   ├── views/                    # 19 个页面（含 knowledge-base / conversation ★v0.9）
│   │   ├── components/               # 7 个通用组件
│   │   ├── api/                      # 16 个 API 模块（含 knowledgeBase / conversation ★v0.9）
│   │   ├── stores/                   # Pinia 状态
│   │   ├── layouts/                  # 主布局
│   │   ├── router/                   # 路由配置（21 条路由）
...
用户在会话中提问
  │
  ▼
[会话关联知识库] 取 conversation.knowledge_bases → kb_ids
  │
  ▼
[加载对话历史] conversation_service.get_history(conv_id, turns=6)
  │
  ▼
[持久化用户消息] ConversationMessage(role=user)
  │
  ▼
[构造 dummy Skill] 内存 Skill（id=0, 空 scope, 不走 Skill 路由）
  │
  ▼
[RAG 检索] rag_service.answer_stream(knowledge_base_ids=kb_ids)
  │   ├─ 仅检索 kb_ids 内 status=published 的知识
  │   ├─ 向量检索 + 真 BM25 + RRF 融合
  │   └─ 来源含 chunk_id + page_number
  │
  ▼
[SSE 流式生成] sources → chunks → done
  │
  ▼
[持久化 AI 回复] ConversationMessage(role=assistant, sources, token, 耗时)
```
...
| 知识库管理 | /knowledge-base | 多知识库容器、独立模型配置、批量上传+解析状态机 ★ v0.9 |
| 知识库详情 | /knowledge-base/detail/:id | 知识库资料列表、上传（含文件夹）、解析状态筛选、批量解析 ★ v0.9 |
| 会话问答 | /conversation | ChatGPT 风格多轮对话、会话关联知识库、SSE 流式、引用溯源 ★ v0.9 |
...
| 知识库 | GET/POST | /api/knowledge-bases | 知识库 CRUD ★ v0.9 |
| 知识库 | GET | /api/knowledge-bases/{id}/documents | 知识库资料列表 ★ v0.9 |
| 知识库 | POST | /api/knowledge-bases/{id}/upload | 上传资料到知识库（含文件夹）★ v0.9 |
| 知识库 | POST | /api/knowledge-bases/{id}/documents/{kid}/parse | 解析单个资料 ★ v0.9 |
| 知识库 | POST | /api/knowledge-bases/{id}/parse-all | 批量解析所有资料 ★ v0.9 |
| 会话 | GET/POST | /api/conversations | 会话 CRUD ★ v0.9 |
| 会话 | GET | /api/conversations/{id} | 会话详情（含消息历史）★ v0.9 |
| 会话 | POST | /api/conversations/{id}/ask | 会话内 SSE 流式问答 ★ v0.9 |
| 会话 | DELETE | /api/conversations/{id}/messages | 清空会话消息 ★ v0.9 |
...
**Q: 知识库和知识管理有什么区别？**
A: 「知识库管理」（v0.9）是顶层容器，用于按业务域（如"设备手册库"、"质量标准库"）组织资料，每个知识库可独立配置 LLM/Embedding/Rerank/OCR/VLM（空则继承全局），支持批量上传和解析状态机。「知识管理」是单条知识条目的 CRUD，可归属到某个知识库（也可不归属，兼容旧数据）。会话可关联多个知识库限定检索范围。

**Q: 会话问答和智能问答有什么区别？**
A: 「智能问答」走 Skill 路由（关键词/语义三级匹配），按 Skill 的 knowledge_scope 限定检索范围，适合单轮或简单多轮。「会话问答」（v0.9）是 ChatGPT 风格的多轮对话，不走 Skill 路由，而是按会话关联的知识库限定检索范围，支持完整对话历史上下文、引用溯源、流式输出。两者底层都复用 rag_service 的混合检索 + RRF 融合。
